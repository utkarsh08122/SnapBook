...
async function profile({ params }: { params: { id: string } }) {
  ...
  try {
    ...
    const posts: any = await Post.find({ owner: id }).populate("owner");
    return (
      <>
        {user ? (
          <div className="w-full mt-14 flex justify-center">
            <div className="w-[95%] flex  justify-evenly">
              ...
              <div className="md:w-[55%] w-full flex flex-col gap-4">
                {user && (
                  <Profilecard key={user._id} type={"user"} user={user} />
                )}

                {/* {posts ?
                 (
                  posts.map((items: any) => {
                    return <Hero key={items._id} type={"home"} post={items} />;
                  })
                )
                 : (
